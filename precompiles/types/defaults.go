...
const jsonPrecompileBaseGas = 40_000
const blake2bhashPrecompileBaseGas = 3_000
const ecvrfPrecompileBaseGas = 50_000
const frostPrecompileBaseGas = 60_000
const gnarkhashPrecompileBaseGas = 6_000
const sha3hashPrecompileBaseGas = 3_000
const poseidonHashBaseGas = 3_000
const pqmldsaBaseGas = 200_000
const pqslhdsaBaseGas = 250_000
const schnorrPrecompileBaseGas = 15_000
const schnorrkelPrecompileBaseGas = 40_000
const ed25519PrecompileBaseGas = 12_000
const sp1verifierGroth16PrecompileBaseGas = 300_000
const sp1verifierPlonkPrecompileBaseGas = 800_000
...
func assertAvailableStaticPrecompilesRegistered(precompiles map[common.Address]vm.PrecompiledContract) {
	for _, addressHex := range evmtypes.AvailableStaticPrecompiles {
		address := common.HexToAddress(addressHex)
		if _, found := precompiles[address]; !found {
			panic(fmt.Errorf("available static precompile %s is not registered", addressHex))
		}
	}
}
