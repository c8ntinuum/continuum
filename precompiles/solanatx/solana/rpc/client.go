...
func (c *RpcClient) Call(ctx context.Context, params ...any) ([]byte, error) {
	...
	// prepare request
	req, err := http.NewRequestWithContext(ctx, "POST", c.endpoint.Url, bytes.NewBuffer(j))
	if err != nil {
		return nil, fmt.Errorf("failed to do http.NewRequestWithContext, err: %v", err)
	}
	req.Header.Add("Content-Type", "application/json")
	if c.endpoint.HeaderApiKey != "" {
		req.Header.Add(c.endpoint.HeaderApiKey, c.endpoint.ApiKey)
	}
