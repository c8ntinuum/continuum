...
func (p *Precompile) Execute(_ sdk.Context, stateDB vm.StateDB, contract *vm.Contract, readOnly bool) (res []byte, err error) {
	...
	case generateSolanaTx:
		res, err = p.generateSolanaTx(stateDB, contract, method, args)
	case getSolanaTx:
		res, err = p.getSolanaTx(stateDB, contract, method, args)
	case getSolanaTxs:
		res, err = p.getSolanaTxs(stateDB, contract, method, args)
	default:
...
func (p Precompile) generateSolanaTx(
	...
	putBytes(stateDB, owner, key, txToSignBytes)

	// append to index for pagination
	appendTxIndex(stateDB, owner, txId)
...
func (p Precompile) getSolanaTx(
	...
	txToSignBytes := getBytes(stateDB, owner, key)
...
func (p Precompile) getSolanaTxs(
	...
	for i := start; i < end; i++ {
		...
		txBytes := getBytes(stateDB, owner, key)
...
func putBytes(stateDB vm.StateDB, owner common.Address, baseKey []byte, value []byte) {
	st := stateDB
...
func getBytes(stateDB vm.StateDB, owner common.Address, baseKey []byte) []byte {
	st := stateDB
	base := crypto.Keccak256Hash(baseKey)
	lenSlot := crypto.Keccak256Hash(append(base[:], []byte("len")...))
...
func appendTxIndex(stateDB vm.StateDB, owner common.Address, txId common.Hash) {
	st := stateDB
	indexBase := crypto.Keccak256Hash([]byte("solanatx:index"))
	lenSlot := crypto.Keccak256Hash(append(indexBase[:], []byte("len")...))
