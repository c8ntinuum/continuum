...
func (p *Precompile) Execute(_ sdk.Context, stateDB vm.StateDB, contract *vm.Contract, readOnly bool) ([]byte, error) {
	...
	case methodAddressExists:
		return p.addressExists(stateDB, contract, method, args)
	case methodCompress:
		return p.compress(stateDB, contract, method, args)
	case methodDecompress:
		return p.decompress(stateDB, contract, method, args)
	case methodLookup:
		return p.lookup(stateDB, contract, method, args)
	case methodLookupIndex:
		return p.lookupIndex(stateDB, contract, method, args)
	case methodRegister:
		return p.register(stateDB, contract, method, args)
	case methodSize:
		return p.size(stateDB, contract, method, args)
	default:
		return nil, fmt.Errorf(cmn.ErrUnknownMethod, method.Name)
	}
}

func getSize(stateDB vm.StateDB, self gethcommon.Address) uint64 {
	h := stateDB.GetState(self, slotSizeKey)
...
func putSize(stateDB vm.StateDB, self gethcommon.Address, n uint64) {
	var b [32]byte
	bi := new(big.Int).SetUint64(n)
	copy(b[32-len(bi.Bytes()):], bi.Bytes())
	stateDB.SetState(self, slotSizeKey, gethcommon.BytesToHash(b[:]))
}
...
func getIndexOf(stateDB vm.StateDB, self gethcommon.Address, a gethcommon.Address) (uint64, bool) {
	slot := addrIndexSlot(a)
	v := stateDB.GetState(self, slot)
...
func setIndexOf(stateDB vm.StateDB, self gethcommon.Address, a gethcommon.Address, index uint64) {
	var b [32]byte
	bi := new(big.Int).SetUint64(index + 1) // store index+1
	copy(b[32-len(bi.Bytes()):], bi.Bytes())
	stateDB.SetState(self, addrIndexSlot(a), gethcommon.BytesToHash(b[:]))
}

func getAddressAt(stateDB vm.StateDB, self gethcommon.Address, index uint64) (gethcommon.Address, bool) {
	slot := indexAddrSlot(index)
	h := stateDB.GetState(self, slot)
	if h == (gethcommon.Hash{}) {
...
func setAddressAt(stateDB vm.StateDB, self gethcommon.Address, index uint64, a gethcommon.Address) {
	var b [32]byte
	copy(b[12:], a.Bytes())
	stateDB.SetState(self, indexAddrSlot(index), gethcommon.BytesToHash(b[:]))
}

func (p *Precompile) addressExists(stateDB vm.StateDB, contract *vm.Contract, m *abi.Method, args []interface{}) ([]byte, error) {
	if len(args) != 1 {
		return nil, vm.ErrExecutionReverted
	}
	addr, ok := args[0].(gethcommon.Address)
	if !ok {
		out, _ := m.Outputs.Pack(false)
		return out, nil
	}
	_, exists := getIndexOf(stateDB, contract.Address(), addr)
	out, _ := m.Outputs.Pack(exists)
	return out, nil
}

func (p *Precompile) size(stateDB vm.StateDB, contract *vm.Contract, m *abi.Method, args []interface{}) ([]byte, error) {
	if len(args) != 0 {
		return nil, vm.ErrExecutionReverted
	}
	out, _ := m.Outputs.Pack(new(big.Int).SetUint64(getSize(stateDB, contract.Address())))
	return out, nil
}

func (p *Precompile) lookup(stateDB vm.StateDB, contract *vm.Contract, m *abi.Method, args []interface{}) ([]byte, error) {
	if len(args) != 1 {
		return nil, vm.ErrExecutionReverted
	}
	addr, ok := args[0].(gethcommon.Address)
	...
	index, exists := getIndexOf(stateDB, contract.Address(), addr)
	if !exists {
...
func (p *Precompile) lookupIndex(stateDB vm.StateDB, contract *vm.Contract, m *abi.Method, args []interface{}) ([]byte, error) {
	if len(args) != 1 {
		return nil, vm.ErrExecutionReverted
	}
	indexBI, ok := args[0].(*big.Int)
	if !ok || indexBI.Sign() < 0 {
		return nil, vm.ErrExecutionReverted
	}
	index := indexBI.Uint64()
	a, exists := getAddressAt(stateDB, contract.Address(), index)
	if !exists {
...
func (p *Precompile) register(stateDB vm.StateDB, contract *vm.Contract, m *abi.Method, args []interface{}) ([]byte, error) {
	if len(args) != 1 {
		return nil, vm.ErrExecutionReverted
	}
	addr, ok := args[0].(gethcommon.Address)
	...
	if idx, exists := getIndexOf(stateDB, contract.Address(), addr); exists {
		out, _ := m.Outputs.Pack(new(big.Int).SetUint64(idx))
		return out, nil
	}

	size := getSize(stateDB, contract.Address())
	setIndexOf(stateDB, contract.Address(), addr, size)
	setAddressAt(stateDB, contract.Address(), size, addr)
	putSize(stateDB, contract.Address(), size+1)

	out, _ := m.Outputs.Pack(new(big.Int).SetUint64(size))
	return out, nil
}

func (p *Precompile) compress(stateDB vm.StateDB, contract *vm.Contract, m *abi.Method, args []interface{}) ([]byte, error) {
	if len(args) != 1 {
		return nil, vm.ErrExecutionReverted
	}
	addr, ok := args[0].(gethcommon.Address)
...
func (p *Precompile) decompress(stateDB vm.StateDB, contract *vm.Contract, m *abi.Method, args []interface{}) ([]byte, error) {
	if len(args) != 2 {
		return nil, vm.ErrExecutionReverted
	}
	buf, ok1 := args[0].([]byte)
	offsetBI, ok2 := args[1].(*big.Int)
	if !(ok1 && ok2) {
		return nil, vm.ErrExecutionReverted
	}
	if offsetBI.Sign() < 0 {
	...
	addr, newOff, derr := decodeCompressed(buf, offset, func(idx uint64) (gethcommon.Address, bool) {
		return getAddressAt(stateDB, contract.Address(), idx)
	})
	if derr != nil {
		return nil, derr
	}
