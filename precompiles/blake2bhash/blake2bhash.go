...
const (
	blake2bPerWordGas = 30
	maxBlake2bInput   = 256 * 1024
)
...
func (p Precompile) RequiredGas(input []byte) uint64 {
	return cmn.LinearRequiredGas(p.baseGas, input, blake2bPerWordGas)
}
...
func (p *Precompile) Run(_ *vm.EVM, contract *vm.Contract, _ bool) (bz []byte, err error) {
	...
	if len(data) > maxBlake2bInput {
		return nil, fmt.Errorf("blake2b input exceeds %d bytes", maxBlake2bInput)
	}
