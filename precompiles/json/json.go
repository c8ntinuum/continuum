...
func (p Precompile) RequiredGas(input []byte) uint64 {
	return cmn.LinearRequiredGas(p.baseGas, input, jsonPerWordGas)
}
...
func (p Precompile) extractAsBytes(
	...
	bz := args[0].([]byte)
	if err := validateJSONPayload(bz); err != nil {
		return nil, err
	}
...
func validateJSONPayload(payload []byte) error {
	if len(payload) > maxJSONInputBytes {
		return fmt.Errorf("json payload exceeds %d bytes", maxJSONInputBytes)
	}

	if err := validateJSONNestingDepth(payload); err != nil {
		return err
	}
...
func validateJSONNestingDepth(payload []byte) error {
	depth := 0
	inString := false
	escaped := false
	...
	for _, b := range payload {
		...
		case '{', '[':
			...
			if depth > maxJSONNestingDepth {
				return fmt.Errorf("json payload exceeds nesting depth %d", maxJSONNestingDepth)
			}
		case '}', ']':
			if depth > 0 {
				depth--
