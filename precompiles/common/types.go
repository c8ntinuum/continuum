...
func NewCoinResponse(amount sdk.Coin) Coin {
	return Coin{
		Denom:  amount.Denom,
...
// ToCoin converts a value returned from the ABI to a Coin.
func ToCoin(v interface{}) (Coin, error) {
	// Fast-path: if ABI already returned []Coin (e.g. in tests) just cast.
	if coin, ok := v.(Coin); ok {
		return coin, nil
	}
...
func NewSdkCoinFromCoin(coin Coin) (sdk.Coin, error) {
	sdkCoin := sdk.Coin{
		Denom:  coin.Denom,
		Amount: math.NewIntFromBigInt(coin.Amount),
	}
...
func NewSdkCoinsFromCoin(coin Coin) (sdk.Coins, error) {
	sdkCoins := make(sdk.Coins, 1)
	sdkCoin := sdk.Coin{
		Denom:  coin.Denom,
		Amount: math.NewIntFromBigInt(coin.Amount),
	}
