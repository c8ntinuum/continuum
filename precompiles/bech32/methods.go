...
const (
	...
	maxBech32AddressLength = 128
	maxBech32PrefixLength  = 32
)
...
func (p Precompile) HexToBech32(
	...
	if len(prefix) > maxBech32PrefixLength {
		return nil, fmt.Errorf("bech32 prefix exceeds %d bytes", maxBech32PrefixLength)
	}
...
func (p Precompile) Bech32ToHex(
	...
	if len(address) > maxBech32AddressLength {
		return nil, fmt.Errorf("bech32 address exceeds %d characters", maxBech32AddressLength)
	}
