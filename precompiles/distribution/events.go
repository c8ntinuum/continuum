...
func coinsAmountOrZero(coins sdk.Coins) *big.Int {
	if len(coins) == 0 {
		return big.NewInt(0)
	}

	return coins[0].Amount.BigInt()
}

func packSingleAmountEventData(event abi.Event, coins sdk.Coins) ([]byte, error) {
	data, err := event.Inputs.NonIndexed().Pack(coinsAmountOrZero(coins))
	if err != nil {
		return nil, fmt.Errorf("failed to pack event data: %w", err)
	}
...
func (p Precompile) EmitWithdrawDelegatorRewardEvent(ctx sdk.Context, stateDB vm.StateDB, delegatorAddress common.Address, validatorAddress string, coins sdk.Coins) error {
	...
	data, err := packSingleAmountEventData(event, coins)
	if err != nil {
		return err
	}
