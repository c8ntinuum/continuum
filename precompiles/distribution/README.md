...
If a withdraw succeeds with zero rewards, it emits `WithdrawDelegatorReward` with
`amount = 0`.
...
If a commission withdraw succeeds with zero payout, it emits
`WithdrawValidatorCommission` with `commission = 0`.
...
If no rewards are available, the call still returns `true` and emits `ClaimRewards` with
`amount = 0`.
