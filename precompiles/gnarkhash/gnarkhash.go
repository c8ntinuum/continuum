...
const (
	gnarkHashPerWordGas = 30
	maxGnarkHashInput   = 256 * 1024
)
...
func (p Precompile) RequiredGas(input []byte) uint64 {
	return cmn.LinearRequiredGas(p.baseGas, input, gnarkHashPerWordGas)
}
...
func (p *Precompile) Run(_ *vm.EVM, contract *vm.Contract, _ bool) (bz []byte, err error) {
	...
	if len(data) > maxGnarkHashInput {
		return nil, fmt.Errorf("gnark hash input exceeds %d bytes", maxGnarkHashInput)
	}
