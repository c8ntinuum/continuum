...
func TestRequiredGasScalesByWords(t *testing.T) {
	...
	require.Equal(t, uint64(6_030), precompile.RequiredGas(make([]byte, 1)))
	require.Equal(t, uint64(6_060), precompile.RequiredGas(make([]byte, 33)))
}
...
func TestRunRejectsOversizedInput(t *testing.T) {
	...
	args, err := method.Inputs.Pack(make([]byte, maxGnarkHashInput+1), "SHA256")
	require.NoError(t, err)
	...
	require.ErrorContains(t, err, "gnark hash input exceeds")
}
