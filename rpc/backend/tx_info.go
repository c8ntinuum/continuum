...
func (b *Backend) GetTransactionReceipt(hash common.Hash) (map[string]interface{}, error) {
	...
	if res.EthTxIndex >= 0 {
		idx := int(res.EthTxIndex) // #nosec G115 -- checked to be non-negative above
		if idx < len(receipts) && receipts[idx].TxHash == hash {
			receipt = receipts[idx]
		}
	}
	if receipt == nil {
		for i := range receipts {
			if receipts[i].TxHash == hash {
				receipt = receipts[i]
				break
			}
		}
	...
	if receipt == nil {
		return nil, fmt.Errorf("receipt not found for tx %s at height %d", hexTx, res.Height)
	}
