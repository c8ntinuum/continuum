...
func NewExperimentalEVMMempool(
	...
	if config == nil {
		panic("config must not be nil") //nolint:halt // Constructor guard: mempool creation requires an explicit config.
	}
	...
	if err != nil {
		panic(err) //nolint:halt // Constructor guard: txpool bootstrap must succeed before the mempool is exposed.
	}

	if len(txPool.Subpools) != 1 {
		panic("tx pool should contain one subpool") //nolint:halt // Internal invariant: continuum only supports a single legacy tx subpool.
	}
	if _, ok := txPool.Subpools[0].(*legacypool.LegacyPool); !ok {
		panic("tx pool should contain only legacypool") //nolint:halt // Internal invariant: continuum mempool wiring assumes the legacy tx subpool implementation.
	}
...
func (m *ExperimentalEVMMempool) SetEventBus(eventBus *cmttypes.EventBus) {
	...
	if err != nil {
		panic(err) //nolint:halt // Internal invariant: block-header subscription must succeed once the event bus is wired.
	}
	go func() {
		for range sub.Out() {
