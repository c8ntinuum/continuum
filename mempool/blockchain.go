...
func (b *Blockchain) GetBlock(_ common.Hash, _ uint64) *types.Block {
	...
	panic("GetBlock should never be called on a Cosmos chain due to instant finality - this indicates a reorg is being attempted") //nolint:halt // Internal invariant: the Cosmos mempool must never request reorg blocks on an instant-finality chain.
}
