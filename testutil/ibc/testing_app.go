...
func SetupTestingApp() (TestingApp, map[string]json.RawMessage) {
	// Avoid stale bech32 cache entries (e.g. c8-prefixed module addresses) when
	// building simapp under cosmos prefixes.
	cacheEnabled := sdk.IsAddrCacheEnabled()
	sdk.SetAddrCacheEnabled(false)
	defer sdk.SetAddrCacheEnabled(cacheEnabled)
...
func setupWithGenesisValSet(tb testing.TB, valSet *cmttypes.ValidatorSet, genAccs []authtypes.GenesisAccount, chainID string, powerReduction sdkmath.Int, appCreator ibctesting.AppCreator, metadata []banktypes.Metadata, balances ...banktypes.Balance) TestingApp {
	...
	if _, isCosmosSimApp := app.(*simapp.SimApp); isCosmosSimApp {
		cacheEnabled := sdk.IsAddrCacheEnabled()
		sdk.SetAddrCacheEnabled(false)
		defer sdk.SetAddrCacheEnabled(cacheEnabled)
	}
	...
	// Keep IBC integration tests deterministic and independent from simapp inflation mechanics.
	mintGenesis := minttypes.DefaultGenesisState()
	mintGenesis.Params.MintDenom = bondDenom
	mintGenesis.Params.InflationRateChange = sdkmath.LegacyZeroDec()
	mintGenesis.Params.InflationMax = sdkmath.LegacyZeroDec()
	mintGenesis.Params.InflationMin = sdkmath.LegacyZeroDec()
	mintGenesis.Minter.Inflation = sdkmath.LegacyZeroDec()
	mintGenesis.Minter.AnnualProvisions = sdkmath.LegacyZeroDec()
