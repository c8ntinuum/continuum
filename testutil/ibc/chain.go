...
// fixedAddressAccount preserves raw address bytes so GetAddress does not depend
// on global bech32 prefix parsing from BaseAccount.Address string.
type fixedAddressAccount struct {
	sdk.AccountI
	addr sdk.AccAddress
}

func newFixedAddressAccount(acc sdk.AccountI, addr sdk.AccAddress) sdk.AccountI {
...
func (acc *fixedAddressAccount) GetAddress() sdk.AccAddress {
	return acc.addr
}

func (acc *fixedAddressAccount) SetAddress(addr sdk.AccAddress) error {
	acc.addr = addr
...
func NewTestChainWithValSet(tb testing.TB, isEVM bool, coord *Coordinator, chainID string, valSet *cmttypes.ValidatorSet, signers map[string]cmttypes.PrivValidator) *TestChain {
	...
	for i := 0; i < MaxAccounts; i++ {
		...
		rawAddr := sdk.AccAddress(senderPrivKey.PubKey().Address().Bytes())
		baseAcc := authtypes.NewBaseAccount(rawAddr, senderPrivKey.PubKey(), uint64(i), 0)
		senderAccAccount := newFixedAddressAccount(baseAcc, rawAddr)
		amount, ok := sdkmath.NewIntFromString(DefaultGenesisAccBalance)
		require.True(tb, ok)

		// add sender account
		balance := banktypes.Balance{
			Address: rawAddr.String(),
			Coins: sdk.NewCoins(
		...
		genAccs = append(genAccs, baseAcc)
		genBals = append(genBals, balance)

		senderAcc := SenderAccount{
			SenderAccount: senderAccAccount,
...
func (chain *TestChain) SendMsgsWithSender(sender SenderAccount, msgs ...sdk.Msg) (*abci.ExecTxResult, error) {
	...
	// SimApp message validation still relies on global bech32 config.
	// Switch prefixes while delivering Cosmos-chain messages.
	if _, isCosmosSimApp := chain.App.(*simapp.SimApp); isCosmosSimApp {
		cacheEnabled := sdk.IsAddrCacheEnabled()
		sdk.SetAddrCacheEnabled(false)
		defer sdk.SetAddrCacheEnabled(cacheEnabled)
		setBech32PrefixesToCosmos()
		defer setBech32PrefixesToEVM()
	}
