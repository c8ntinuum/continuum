...
LOGLEVEL="info"
# Set dedicated home directory for the ~/go/bin/ctmd instance
CHAINDIR="$HOME/.ctmd"
...
Options:
  ...
  --mnemonic-file PATH     Where to write mnemonics YAML (default: \$HOME/.~/go/bin/ctmd/mnemonics.yaml)
...
while [[ $# -gt 0 ]]; do
  ...
  case $key in
    ...
    -n)
      ...
      ;;
    --no-install)
      echo "Flag --no-install passed -> Skipping installation of the ~/go/bin/ctmd binary."
      install=false; shift
      ;;
    --remote-debugging)
...
add_genesis_funds() {
  local keyname="$1"
  ~/go/bin/ctmd genesis add-genesis-account "$keyname" 1000000000000000000000ctm --keyring-backend "$KEYRING" --home "$CHAINDIR"
}
...
if [[ $overwrite == "y" || $overwrite == "Y" ]]; then
  ...
  ~/go/bin/ctmd config set client chain-id "$CHAINID" --home "$CHAINDIR"
  ~/go/bin/ctmd config set client keyring-backend "$KEYRING" --home "$CHAINDIR"

  # ---------------- Validator key ----------------
  VAL_KEY="mykey"
  VAL_MNEMONIC="gesture inject test cycle original hollow east ridge hen combine junk child bacon zero hope comfort vacuum milk pitch cage oppose unhappy lunar seat"
  echo "$VAL_MNEMONIC" | ~/go/bin/ctmd keys add "$VAL_KEY" --recover --keyring-backend "$KEYRING" --algo "$KEYALGO" --home "$CHAINDIR"
  ...
  echo "$VAL_MNEMONIC" | ~/go/bin/ctmd init $MONIKER -o --chain-id "$CHAINID" --home "$CHAINDIR" --recover

  # ---------- Genesis customizations ----------
  jq '.app_state["staking"]["params"]["bond_denom"]="ctm"' "$GENESIS" >"$TMP_GENESIS" && mv "$TMP_GENESIS" "$GENESIS"
  jq '.app_state["gov"]["deposit_params"]["min_deposit"][0]["denom"]="ctm"' "$GENESIS" >"$TMP_GENESIS" && mv "$TMP_GENESIS" "$GENESIS"
  jq '.app_state["gov"]["params"]["min_deposit"][0]["denom"]="ctm"' "$GENESIS" >"$TMP_GENESIS" && mv "$TMP_GENESIS" "$GENESIS"
  jq '.app_state["gov"]["params"]["expedited_min_deposit"][0]["denom"]="ctm"' "$GENESIS" >"$TMP_GENESIS" && mv "$TMP_GENESIS" "$GENESIS"
  jq '.app_state["evm"]["params"]["evm_denom"]="ctm"' "$GENESIS" >"$TMP_GENESIS" && mv "$TMP_GENESIS" "$GENESIS"
  jq '.app_state["mint"]["params"]["mint_denom"]="ctm"' "$GENESIS" >"$TMP_GENESIS" && mv "$TMP_GENESIS" "$GENESIS"

  jq '.app_state["bank"]["denom_metadata"]=[{"description":"The native staking token for ~/go/bin/ctmd.","denom_units":[{"denom":"ctm","exponent":0,"aliases":["attotest"]},{"denom":"test","exponent":18,"aliases":[]}],"base":"ctm","display":"test","name":"Test Token","symbol":"TEST","uri":"","uri_hash":""}]' "$GENESIS" >"$TMP_GENESIS" && mv "$TMP_GENESIS" "$GENESIS"

  jq '.app_state["evm"]["params"]["active_static_precompiles"]=["0x0000000000000000000000000000000000000100","0x0000000000000000000000000000000000000400","0x0000000000000000000000000000000000000800","0x0000000000000000000000000000000000000801","0x0000000000000000000000000000000000000802","0x0000000000000000000000000000000000000804","0x0000000000000000000000000000000000000805", "0x0000000000000000000000000000000000000806", "0x0000000000000000000000000000000000000807"]' "$GENESIS" >"$TMP_GENESIS" && mv "$TMP_GENESIS" "$GENESIS"
  ...
  jq '.app_state.erc20.token_pairs=[{contract_owner:1,erc20_address:"0xEeeeeEeeeEeEeeEeEeEeeEEEeeeeEeeeeeeeEEeE",denom:"ctm",enabled:true}]' "$GENESIS" >"$TMP_GENESIS" && mv "$TMP_GENESIS" "$GENESIS"
  ...
  ~/go/bin/ctmd genesis add-genesis-account "$VAL_KEY" 100000000000000000000000000ctm --keyring-backend "$KEYRING" --home "$CHAINDIR"
  ...
  sed -i.bak 's/timeout_commit = "5s"/timeout_commit = "4s"/g' "$CONFIG_TOML"
  ...
  for ((i=0; i<${#dev_mnemonics[@]}; i++)); do
    ...
    echo "$mnemonic" | ~/go/bin/ctmd keys add "$keyname" --recover --keyring-backend "$KEYRING" --algo "$KEYALGO" --home "$CHAINDIR"
  ...
  if [[ "$ADDITIONAL_USERS" -gt 0 ]]; then
    ...
    for ((i=0; i<ADDITIONAL_USERS; i++)); do
      ...
      mnemonic_out="$(~/go/bin/ctmd keys add "$keyname" --keyring-backend "$KEYRING" --algo "$KEYALGO" --home "$CHAINDIR" 2>&1)"
  ...
  # --------- Finalize genesis ---------
  ~/go/bin/ctmd genesis gentx "$VAL_KEY" 1000000000000000000000000ctm --min-self-delegation 888888000000000000000000 --gas-prices ${BASEFEE}ctm --keyring-backend "$KEYRING" --chain-id "$CHAINID" --home "$CHAINDIR"
  ~/go/bin/ctmd genesis collect-gentxs --home "$CHAINDIR"
  ~/go/bin/ctmd genesis validate-genesis --home "$CHAINDIR"
...
# Start the node
~/go/bin/ctmd start "$TRACE" \
	--pruning nothing \
	--log_level $LOGLEVEL \
	--minimum-gas-prices=0ctm \
	--evm.min-tip=0 \
	--home "$CHAINDIR" \
