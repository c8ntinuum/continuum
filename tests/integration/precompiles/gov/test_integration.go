	...
	_ = Describe("Calling governance precompile from EOA", func() {
		...
		Describe("Execute SubmitProposal transaction", func() {
			...
			It("fails with low gas", func() {
				...
				txArgs.GasLimit = estimateLowGas(proposerAddr)

				moduleBalanceBefore := s.network.App.GetBankKeeper().GetBalance(s.network.GetContext(), govModuleAddr, s.network.GetBaseDenom())

				_, _, err := s.factory.CallContractAndCheckLogs(proposerKey, txArgs, callArgs, outOfGasCheck)
				Expect(err).To(BeNil())

				_, err = s.network.App.GetGovKeeper().Proposals.Get(s.network.GetContext(), 3)
				Expect(err).To(HaveOccurred())

				moduleBalanceAfter := s.network.App.GetBankKeeper().GetBalance(s.network.GetContext(), govModuleAddr, s.network.GetBaseDenom())
				Expect(moduleBalanceAfter).To(Equal(moduleBalanceBefore))
			})
		...
		Describe("Execute Deposit transaction", func() {
			...
			It("fails with low gas and keeps deposits unchanged", func() {
				callArgs.Args = []interface{}{proposerAddr, proposalID, minimalDeposit(s.network.GetBaseDenom(), big.NewInt(1))}
				txArgs.GasLimit = estimateLowGas(proposerAddr)

				proposalBefore, err := s.network.App.GetGovKeeper().Proposals.Get(s.network.GetContext(), proposalID)
				Expect(err).To(BeNil())
				moduleBalanceBefore := s.network.App.GetBankKeeper().GetBalance(s.network.GetContext(), govModuleAddr, s.network.GetBaseDenom())
				...
				proposalAfter, err := s.network.App.GetGovKeeper().Proposals.Get(s.network.GetContext(), proposalID)
				Expect(err).To(BeNil())
				Expect(proposalAfter.GetTotalDeposit()).To(Equal(proposalBefore.GetTotalDeposit()))
		...
		Describe("Execute CancelProposal transaction", func() {
			...
			It("fails with low gas and keeps the proposal live", func() {
				callArgs.Args = []interface{}{proposerAddr, proposalID}
				txArgs.GasLimit = estimateLowGas(proposerAddr)
				...
				proposalAfter, err := s.network.App.GetGovKeeper().Proposals.Get(s.network.GetContext(), proposalID)
				Expect(err).To(BeNil())
				Expect(proposalAfter.Status).To(Equal(proposalBefore.Status))
