...
func TestPrecompileIntegrationTestSuite(t *testing.T, create network.CreateEvmApp, options ...network.ConfigOption) {
	_ = Describe("Calling staking precompile directly", func() {
		...
		estimateLowGas := func(from common.Address) uint64 {
			...
			estimatedGas, err := s.factory.EstimateGasLimit(&from, &estimatedTx)
			Expect(err).To(BeNil())
			Expect(estimatedGas).To(BeNumerically(">", 1))

			return estimatedGas - 1
		}
		...
		Describe("Revert transaction", func() {
			It("should run out of gas if the gas limit is too low", func() {
				delegator := s.keyring.GetKey(0)
				delegationBefore, err := s.grpcHandler.GetDelegation(delegator.AccAddr.String(), valAddr.String())
				Expect(err).To(BeNil())
				...
				txArgs.GasLimit = estimateLowGas(delegator.Addr)
				...
				delegationAfter, err := s.grpcHandler.GetDelegation(delegator.AccAddr.String(), valAddr.String())
				Expect(err).To(BeNil())
				Expect(delegationAfter.DelegationResponse.Delegation.GetShares()).To(Equal(delegationBefore.DelegationResponse.Delegation.GetShares()))
			})
		})
		...
		Describe("to create validator", func() {
			...
			Context("when validator address is the msg.sender & EoA", func() {
				It("should revert create validator on low gas without leaving state behind", func() {
					callArgs.Args = []interface{}{
						defaultDescription, defaultCommission, defaultMinSelfDelegation, s.keyring.GetAddr(0), defaultPubkeyBase64Str, defaultValue,
					}
					txArgs.GasLimit = estimateLowGas(s.keyring.GetAddr(0))

					valOperAddr := sdk.ValAddress(s.keyring.GetAccAddr(0)).String()
					_, err := s.network.GetStakingClient().Validator(s.network.GetContext(), &stakingtypes.QueryValidatorRequest{ValidatorAddr: valOperAddr})
		...
		Describe("to edit validator", func() {
			...
			Context("when msg.sender is equal to validator address", func() {
				It("should keep validator metadata unchanged on low gas", func() {
					...
					txArgs.GasLimit = estimateLowGas(hexAddr)

					valOperAddr := sdk.ValAddress(newAddr.Bytes()).String()
					validatorBefore, err := s.network.GetStakingClient().Validator(s.network.GetContext(), &stakingtypes.QueryValidatorRequest{ValidatorAddr: valOperAddr})
					Expect(err).To(BeNil())
					...
					validatorAfter, err := s.network.GetStakingClient().Validator(s.network.GetContext(), &stakingtypes.QueryValidatorRequest{ValidatorAddr: valOperAddr})
					Expect(err).To(BeNil())
					Expect(validatorAfter.Validator.Description).To(Equal(validatorBefore.Validator.Description))
					Expect(validatorAfter.Validator.Commission).To(Equal(validatorBefore.Validator.Commission))
					Expect(validatorAfter.Validator.MinSelfDelegation.String()).To(Equal(validatorBefore.Validator.MinSelfDelegation.String()))
				})
		...
		Describe("to undelegate", func() {
			...
			Context("as the token owner", func() {
				It("should not leave an unbonding entry on low gas", func() {
					delegator := s.keyring.GetKey(0)
					delegationBefore, err := s.grpcHandler.GetDelegation(delegator.AccAddr.String(), valAddr.String())
					Expect(err).To(BeNil())
					unbondingsBefore, err := s.grpcHandler.GetDelegatorUnbondingDelegations(delegator.AccAddr.String())
					Expect(err).To(BeNil())
					...
					unbondingsAfter, err := s.grpcHandler.GetDelegatorUnbondingDelegations(delegator.AccAddr.String())
					Expect(err).To(BeNil())
					Expect(unbondingsAfter.UnbondingResponses).To(HaveLen(len(unbondingsBefore.UnbondingResponses)))
				})
		...
		Describe("to redelegate", func() {
			...
			Context("as the token owner", func() {
				It("should not leave a redelegation entry on low gas", func() {
					delegator := s.keyring.GetKey(0)
					delegationBefore, err := s.grpcHandler.GetDelegation(delegator.AccAddr.String(), valAddr.String())
					Expect(err).To(BeNil())
					redelegationsBefore, err := s.grpcHandler.GetRedelegations(delegator.AccAddr.String(), valAddr.String(), valAddr2.String())
					Expect(err).To(BeNil())
					...
					redelegationsAfter, err := s.grpcHandler.GetRedelegations(delegator.AccAddr.String(), valAddr.String(), valAddr2.String())
					Expect(err).To(BeNil())
					Expect(redelegationsAfter.RedelegationResponses).To(HaveLen(len(redelegationsBefore.RedelegationResponses)))
				})
		...
		Describe("to cancel an unbonding delegation", func() {
			...
			Context("as the token owner", func() {
				It("should keep the unbonding entry intact on low gas", func() {
					delegator := s.keyring.GetKey(0)
					creationHeight := s.network.GetContext().BlockHeight()
					callArgs.Args = []interface{}{
					...
					unbondingsBefore, err := s.grpcHandler.GetDelegatorUnbondingDelegations(delegator.AccAddr.String())
					Expect(err).To(BeNil())
					valDelBefore, err := s.grpcHandler.GetValidatorDelegations(s.network.GetValidators()[0].GetOperator())
					Expect(err).To(BeNil())
					...
					Expect(err).To(BeNil())
					Expect(unbondingsAfter.UnbondingResponses).To(HaveLen(len(unbondingsBefore.UnbondingResponses)))
					Expect(unbondingsAfter.UnbondingResponses[0].Entries).To(HaveLen(len(unbondingsBefore.UnbondingResponses[0].Entries)))

					valDelAfter, err := s.grpcHandler.GetValidatorDelegations(s.network.GetValidators()[0].GetOperator())
					Expect(err).To(BeNil())
					Expect(valDelAfter.DelegationResponses).To(HaveLen(len(valDelBefore.DelegationResponses)))
				})
