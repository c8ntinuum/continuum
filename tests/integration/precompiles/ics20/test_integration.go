...
func TestPrecompileIntegrationTestSuite(t *testing.T, evmAppCreator ibctesting.AppCreator) {
	...
	_ = Describe("Calling ICS20 precompile from callerContract", func() {
		...
		It("should fail if the provided gas limit is too low", func() {
			...
			contractBalanceBefore := evmAppA.GetBankKeeper().GetBalance(
				s.chainA.GetContext(),
				ics20CallerAddr.Bytes(),
				sourceBondDenom,
			)
			escrowBalanceBefore := evmAppA.GetBankKeeper().GetBalance(
				s.chainA.GetContext(),
				escrowAddr,
				sourceBondDenom,
			)
			nextSeqBefore, found := evmAppA.GetIBCKeeper().ChannelKeeper.GetNextSequenceSend(s.chainA.GetContext(), sourcePortID, sourceChannelID)
			...
			escrowBalanceAfter := evmAppA.GetBankKeeper().GetBalance(
				s.chainA.GetContext(),
				escrowAddr,
				sourceBondDenom,
			)
			...
			nextSeqAfter, found := evmAppA.GetIBCKeeper().ChannelKeeper.GetNextSequenceSend(s.chainA.GetContext(), sourcePortID, sourceChannelID)
			Expect(found).To(BeTrue())
			Expect(nextSeqAfter).To(Equal(nextSeqBefore))
		})
