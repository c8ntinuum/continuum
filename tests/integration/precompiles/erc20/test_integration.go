...
func TestIntegrationTestSuite(t *testing.T, create network.CreateEvmApp, options ...network.ConfigOption) {
	...
	_ = Describe("ERC20 Extension -", func() {
		...
		Context("OOG rollback -", func() {
			estimateLowGas := func(from common.Address, txArgs evmtypes.EvmTxArgs, callArgs testutiltypes.CallArgs) uint64 {
				input, err := factory.GenerateContractCallArgs(callArgs)
				Expect(err).ToNot(HaveOccurred())
				txArgs.Input = input

				estimatedGas, err := is.factory.EstimateGasLimit(&from, &txArgs)
				Expect(err).ToNot(HaveOccurred())
			...
			It("keeps balances unchanged on low-gas transfer", func() {
				sender := is.keyring.GetKey(0)
				receiver := is.keyring.GetKey(1)
				transferAmount := big.NewInt(10)

				senderBalanceBefore, err := is.handler.GetBalanceFromBank(sender.AccAddr, is.tokenDenom)
				Expect(err).ToNot(HaveOccurred())
				receiverBalanceBefore, err := is.handler.GetBalanceFromBank(receiver.AccAddr, is.tokenDenom)
				Expect(err).ToNot(HaveOccurred())

				txArgs, callArgs := is.getTxAndCallArgs(directCall, contractsData, erc20.TransferMethod, receiver.Addr, transferAmount)
				txArgs.GasLimit = estimateLowGas(sender.Addr, txArgs, callArgs)

				outOfGasCheck := failCheck.WithErrContains(vm.ErrOutOfGas.Error())
				_, _, err = is.factory.CallContractAndCheckLogs(sender.Priv, txArgs, callArgs, outOfGasCheck)
				Expect(err).ToNot(HaveOccurred())

				senderBalanceAfter, err := is.handler.GetBalanceFromBank(sender.AccAddr, is.tokenDenom)
				Expect(err).ToNot(HaveOccurred())
				Expect(senderBalanceAfter.Balance).To(Equal(senderBalanceBefore.Balance))

				receiverBalanceAfter, err := is.handler.GetBalanceFromBank(receiver.AccAddr, is.tokenDenom)
				Expect(err).ToNot(HaveOccurred())
				Expect(receiverBalanceAfter.Balance).To(Equal(receiverBalanceBefore.Balance))
			})

			It("keeps allowances unchanged on low-gas approve", func() {
				owner := is.keyring.GetKey(0)
				spender := is.keyring.GetKey(1)
				approveAmount := big.NewInt(100)

				allowanceBefore, err := is.network.App.GetErc20Keeper().GetAllowance(is.network.GetContext(), is.precompile.Address(), owner.Addr, spender.Addr)
				Expect(err).ToNot(HaveOccurred())

				txArgs, callArgs := is.getTxAndCallArgs(directCall, contractsData, erc20.ApproveMethod, spender.Addr, approveAmount)
				txArgs.GasLimit = estimateLowGas(owner.Addr, txArgs, callArgs)

				outOfGasCheck := failCheck.WithErrContains(vm.ErrOutOfGas.Error())
				_, _, err = is.factory.CallContractAndCheckLogs(owner.Priv, txArgs, callArgs, outOfGasCheck)
				Expect(err).ToNot(HaveOccurred())

				allowanceAfter, err := is.network.App.GetErc20Keeper().GetAllowance(is.network.GetContext(), is.precompile.Address(), owner.Addr, spender.Addr)
				Expect(err).ToNot(HaveOccurred())
				Expect(allowanceAfter).To(Equal(allowanceBefore))
			})

			It("keeps balances and allowance unchanged on low-gas transferFrom", func() {
				owner := is.keyring.GetKey(0)
				spender := is.keyring.GetKey(1)
				transferAmount := big.NewInt(10)

				is.setAllowanceForContract(directCall, contractsData, owner.Priv, spender.Addr, big.NewInt(100))

				allowanceBefore, err := is.network.App.GetErc20Keeper().GetAllowance(is.network.GetContext(), is.precompile.Address(), owner.Addr, spender.Addr)
				Expect(err).ToNot(HaveOccurred())
				ownerBalanceBefore, err := is.handler.GetBalanceFromBank(owner.AccAddr, is.tokenDenom)
				Expect(err).ToNot(HaveOccurred())
				spenderBalanceBefore, err := is.handler.GetBalanceFromBank(spender.AccAddr, is.tokenDenom)
				Expect(err).ToNot(HaveOccurred())

				txArgs, callArgs := is.getTxAndCallArgs(directCall, contractsData, erc20.TransferFromMethod, owner.Addr, spender.Addr, transferAmount)
				txArgs.GasLimit = estimateLowGas(spender.Addr, txArgs, callArgs)

				outOfGasCheck := failCheck.WithErrContains(vm.ErrOutOfGas.Error())
				_, _, err = is.factory.CallContractAndCheckLogs(spender.Priv, txArgs, callArgs, outOfGasCheck)
				Expect(err).ToNot(HaveOccurred())
				...
				ownerBalanceAfter, err := is.handler.GetBalanceFromBank(owner.AccAddr, is.tokenDenom)
				Expect(err).ToNot(HaveOccurred())
				Expect(ownerBalanceAfter.Balance).To(Equal(ownerBalanceBefore.Balance))

				spenderBalanceAfter, err := is.handler.GetBalanceFromBank(spender.AccAddr, is.tokenDenom)
				Expect(err).ToNot(HaveOccurred())
				Expect(spenderBalanceAfter.Balance).To(Equal(spenderBalanceBefore.Balance))
			})
		})
