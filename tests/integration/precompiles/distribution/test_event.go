...
func (s *PrecompileTestSuite) TestWithdrawDelegatorRewardEvent() {
	...
	testCases := []struct {
		...
		{
			"success - zero rewards emit a zero-amount event",
			func(val stakingtypes.Validator) []interface{} {
				var err error
				...
				_, err = s.network.App.GetDistrKeeper().WithdrawDelegationRewards(ctx, s.keyring.GetAccAddr(0), valAddr)
				s.Require().NoError(err)
			...
			func() {
				...
				s.Require().Equal(s.keyring.GetAddr(0), delegatorRewards.DelegatorAddress)
				s.Require().Equal(optHexAddr, delegatorRewards.ValidatorAddress)
				s.Require().Zero(delegatorRewards.Amount.Sign())
			},
			20000,
			false,
			"",
...
func (s *PrecompileTestSuite) TestEmitWithdrawDelegatorRewardEventEmptyCoins() {
	s.SetupTest()
	...
	err := s.precompile.EmitWithdrawDelegatorRewardEvent(ctx, stDB, s.keyring.GetAddr(0), validator.OperatorAddress, sdk.Coins{})
	s.Require().NoError(err)
	...
	err = cmn.UnpackLog(s.precompile.ABI, &event, distribution.EventTypeWithdrawDelegatorReward, *logs[0])
	s.Require().NoError(err)
	s.Require().Zero(event.Amount.Sign())
}

func (s *PrecompileTestSuite) TestEmitWithdrawValidatorCommissionEventEmptyCoins() {
	s.SetupTest()
	...
	err := s.precompile.EmitWithdrawValidatorCommissionEvent(ctx, stDB, validator.OperatorAddress, sdk.Coins{})
	s.Require().NoError(err)

	logs := stDB.Logs()
	s.Require().Len(logs, 1)

	var event distribution.EventWithdrawValidatorRewards
	err = cmn.UnpackLog(s.precompile.ABI, &event, distribution.EventTypeWithdrawValidatorCommission, *logs[0])
	s.Require().NoError(err)
	s.Require().Zero(event.Commission.Sign())
}
...
func (s *PrecompileTestSuite) TestClaimRewardsEvent() {
	...
	testCases := []struct {
		...
		{
			"success - empty rewards emit zero",
			sdk.Coins{},
			func() {
				log := stDB.Logs()[0]
				s.Require().Equal(log.Address, s.precompile.Address())
				...
				s.Require().NoError(err)
				s.Require().Equal(common.BytesToAddress(s.keyring.GetAddr(0).Bytes()), claimRewardsEvent.DelegatorAddress)
				s.Require().Zero(claimRewardsEvent.Amount.Sign())
			},
		},
	}
