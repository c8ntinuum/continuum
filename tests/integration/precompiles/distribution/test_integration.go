...
func TestPrecompileIntegrationTestSuite(t *testing.T, create network.CreateEvmApp, options ...network.ConfigOption) {
	_ = Describe("Calling distribution precompile from EOA", func() {
		...
		Describe("Execute WithdrawDelegatorReward transaction", func() {
			...
			It("should keep rewards unchanged on low gas", func() {
				callArgs.Args = []interface{}{
					s.keyring.GetAddr(0),
					s.network.GetValidators()[0].OperatorAddress,
				}
				...
				balanceBefore, err := s.grpcHandler.GetBalanceFromBank(s.keyring.GetAccAddr(0), s.bondDenom)
				Expect(err).To(BeNil())
				rewardsBefore, err := s.grpcHandler.GetDelegationRewards(s.keyring.GetAccAddr(0).String(), s.network.GetValidators()[0].OperatorAddress)
				Expect(err).To(BeNil())
				...
				balanceAfter, err := s.grpcHandler.GetBalanceFromBank(s.keyring.GetAccAddr(0), s.bondDenom)
				Expect(err).To(BeNil())
				Expect(balanceAfter.Balance).To(Equal(balanceBefore.Balance))

				rewardsAfter, err := s.grpcHandler.GetDelegationRewards(s.keyring.GetAccAddr(0).String(), s.network.GetValidators()[0].OperatorAddress)
				Expect(err).To(BeNil())
		...
		Describe("Validator Commission: Execute WithdrawValidatorCommission tx", func() {
			...
			It("should return error if the provided gasLimit is too low", func() {
				...
				txArgs.GasLimit = estimateLowGas(common.BytesToAddress(s.validatorsKeys[0].AccAddr))

				balanceBefore, err := s.grpcHandler.GetBalanceFromBank(s.validatorsKeys[0].AccAddr, s.bondDenom)
				Expect(err).To(BeNil())
				commissionBefore, err := s.grpcHandler.GetValidatorCommission(s.network.GetValidators()[0].OperatorAddress)
				Expect(err).To(BeNil())
				...
				balanceAfter, err := s.grpcHandler.GetBalanceFromBank(s.validatorsKeys[0].AccAddr, s.bondDenom)
				Expect(err).To(BeNil())
				Expect(balanceAfter.Balance).To(Equal(balanceBefore.Balance))

				commissionAfter, err := s.grpcHandler.GetValidatorCommission(s.network.GetValidators()[0].OperatorAddress)
				Expect(err).To(BeNil())
				Expect(commissionAfter.Commission.Commission).To(Equal(commissionBefore.Commission.Commission))
			})
		...
		Describe("Execute ClaimRewards transaction", func() {
			...
			It("should keep rewards and balances unchanged on low gas", func() {
				valCount := len(s.network.GetValidators())
				callArgs.Args = []interface{}{
				...
				balanceBefore, err := s.grpcHandler.GetBalanceFromBank(s.keyring.GetAccAddr(0), s.bondDenom)
				Expect(err).To(BeNil())
				rewardsBefore, err := s.grpcHandler.GetDelegationTotalRewards(s.keyring.GetAccAddr(0).String())
				Expect(err).To(BeNil())
				...
				rewardsAfter, err := s.grpcHandler.GetDelegationTotalRewards(s.keyring.GetAccAddr(0).String())
				Expect(err).To(BeNil())
				Expect(rewardsAfter.Total).To(Equal(rewardsBefore.Total))
			})
		...
		Describe("Execute DepositValidatorRewardsPool transaction", func() {
			...
			It("should keep balances and outstanding rewards unchanged on low gas", func() {
				callArgs.Args = []interface{}{
					s.keyring.GetAddr(0),
					s.network.GetValidators()[0].OperatorAddress,
					[]cmn.Coin{{Denom: s.bondDenom, Amount: big.NewInt(1_000_000)}},
				}
				txArgs.GasLimit = estimateLowGas(s.keyring.GetAddr(0))

				balanceBefore, err := s.grpcHandler.GetBalanceFromBank(s.keyring.GetAccAddr(0), s.bondDenom)
				Expect(err).To(BeNil())
				outstandingBefore, err := s.grpcHandler.GetValidatorOutstandingRewards(s.network.GetValidators()[0].OperatorAddress)
				Expect(err).To(BeNil())
				...
				outstandingAfter, err := s.grpcHandler.GetValidatorOutstandingRewards(s.network.GetValidators()[0].OperatorAddress)
				Expect(err).To(BeNil())
				Expect(outstandingAfter.Rewards.Rewards).To(Equal(outstandingBefore.Rewards.Rewards))
			})
		...
		Describe("Execute FundCommunityPool transaction", func() {
			...
			It("should keep balances and community pool unchanged on low gas", func() {
				fundAmt := math.NewInt(10)
				callArgs.Args = []interface{}{
					s.keyring.GetAddr(0),
					[]cmn.Coin{{Denom: s.bondDenom, Amount: fundAmt.BigInt()}},
				}
				txArgs.GasLimit = estimateLowGas(s.keyring.GetAddr(0))

				balanceBefore, err := s.grpcHandler.GetBalanceFromBank(s.keyring.GetAccAddr(0), s.bondDenom)
				Expect(err).To(BeNil())
				poolBefore, err := s.grpcHandler.GetCommunityPool()
				Expect(err).To(BeNil())
				...
				poolAfter, err := s.grpcHandler.GetCommunityPool()
				Expect(err).To(BeNil())
				Expect(poolAfter.Pool).To(Equal(poolBefore.Pool))
			})
