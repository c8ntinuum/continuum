...
func (s *PrecompileTestSuite) TestWithdrawDelegatorRewardNoopSecondCall() {
	s.SetupTest()
	...
	contract, ctx := testutil.NewPrecompileContract(s.T(), ctx, s.keyring.GetAddr(0), s.precompile.Address(), 20000)
	...
	_, err = s.precompile.WithdrawDelegatorReward(ctx, contract, stateDB, &method, args)
	s.Require().NoError(err)

	bz, err := s.precompile.WithdrawDelegatorReward(ctx, contract, stateDB, &method, args)
	s.Require().NoError(err)

	var coins []cmn.Coin
	err = s.precompile.UnpackIntoInterface(&coins, distribution.WithdrawDelegatorRewardMethod, bz)
	s.Require().NoError(err)
	s.Require().Len(coins, 1)
	s.Require().NotEmpty(coins[0].Denom)
	s.Require().Zero(coins[0].Amount.Sign())

	logs := stateDB.Logs()
	s.Require().Len(logs, 2)

	var event distribution.EventWithdrawDelegatorReward
	err = cmn.UnpackLog(s.precompile.ABI, &event, distribution.EventTypeWithdrawDelegatorReward, *logs[1])
	s.Require().NoError(err)
	s.Require().Zero(event.Amount.Sign())
}
...
func (s *PrecompileTestSuite) TestClaimRewardsNoopSecondCall() {
	s.SetupTest()
	...
	method := s.precompile.Methods[distribution.ClaimRewardsMethod]
	addr := s.keyring.GetAddr(0)
	contract, ctx := testutil.NewPrecompileContract(s.T(), ctx, addr, s.precompile.Address(), 20000)
	...
	args := []interface{}{
		addr,
		uint32(len(validators)), //#nosec G115 -- test validator count fits uint32
	}

	_, err = s.precompile.ClaimRewards(ctx, contract, stateDB, &method, args)
	s.Require().NoError(err)

	bz, err := s.precompile.ClaimRewards(ctx, contract, stateDB, &method, args)
	s.Require().NoError(err)
	s.Require().Equal(cmn.TrueValue, bz)

	logs := stateDB.Logs()
	s.Require().Len(logs, 2)

	var event distribution.EventClaimRewards
	err = cmn.UnpackLog(s.precompile.ABI, &event, distribution.EventTypeClaimRewards, *logs[1])
	s.Require().NoError(err)
	s.Require().Zero(event.Amount.Sign())
}
