...
func buildSignedEthereumTxWithNonce(s *TestSuite, nonce uint64) (*evmtypes.MsgEthereumTx, []byte) {
...
func (s *TestSuite) TestGetTransactionReceiptUsesIndexedTransactionPosition() {
	height := int64(1)
	firstMsg, firstTxBz := buildSignedEthereumTxWithNonce(s, 0)
	secondMsg, secondTxBz := buildSignedEthereumTxWithNonce(s, 1)

	client := s.backend.ClientCtx.Client.(*mocks.Client)
	queryClient := s.backend.QueryClient.QueryClient.(*mocks.EVMQueryClient)

	resBlock := RegisterBlockMultipleTxs(client, height, []types.Tx{firstTxBz, secondTxBz})
	blockRes, err := RegisterBlockResultsWithEventLog(client, height)
	s.Require().NoError(err)

	logData := blockRes.TxsResults[0].Data
	blockRes.TxsResults = []*abci.ExecTxResult{
		{
			Code:    0,
			GasUsed: 21000,
			Data:    logData,
			Events: []abci.Event{
				{Type: evmtypes.EventTypeEthereumTx, Attributes: []abci.EventAttribute{
					{Key: evmtypes.AttributeKeyEthereumTxHash, Value: firstMsg.Hash().Hex()},
					{Key: evmtypes.AttributeKeyTxIndex, Value: "0"},
					{Key: evmtypes.AttributeKeyTxGasUsed, Value: "21000"},
				}},
			},
		},
		{
			...
			Events: []abci.Event{
				{Type: evmtypes.EventTypeEthereumTx, Attributes: []abci.EventAttribute{
					{Key: evmtypes.AttributeKeyEthereumTxHash, Value: secondMsg.Hash().Hex()},
					{Key: evmtypes.AttributeKeyTxIndex, Value: "1"},
					{Key: evmtypes.AttributeKeyTxGasUsed, Value: "34123"},
				}},
			},
		},
	}
	...
	firstReceipt, err := s.backend.GetTransactionReceipt(firstMsg.Hash())
	s.Require().NoError(err)
	s.Require().Equal(hexutil.Uint64(0), firstReceipt["transactionIndex"])
	s.Require().Equal(hexutil.Uint64(21000), firstReceipt["cumulativeGasUsed"])

	secondReceipt, err := s.backend.GetTransactionReceipt(secondMsg.Hash())
	s.Require().NoError(err)
	s.Require().Equal(hexutil.Uint64(1), secondReceipt["transactionIndex"])
	s.Require().Equal(hexutil.Uint64(55123), secondReceipt["cumulativeGasUsed"])
}
