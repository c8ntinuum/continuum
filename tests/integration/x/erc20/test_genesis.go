...
func (s *GenesisTestSuite) TestERC20InitGenesisPanicsOnDuplicateTokenPair() {
	erc20Keeper := s.network.App.GetErc20Keeper()
	genesisState := types.NewGenesisState(
		types.DefaultParams(),
		[]types.TokenPair{
			types.NewTokenPair(common.HexToAddress("0x1000000000000000000000000000000000000001"), "uhalt", types.OWNER_MODULE),
			types.NewTokenPair(common.HexToAddress("0x1000000000000000000000000000000000000002"), "uhalt", types.OWNER_MODULE),
		},
		nil,
	)

	s.Require().Panics(func() {
		erc20.InitGenesis(s.network.GetContext(), *erc20Keeper, s.network.App.GetAccountKeeper(), genesisState)
	})
}
...
func (s *GenesisTestSuite) TestErc20ExportGenesis() {
	...
	for _, tc := range testGenCases {
		...
		erc20.InitGenesis(nw.GetContext(), *erc20Keeper, nw.App.GetAccountKeeper(), tc.genesisState)
		s.Require().NotPanics(func() {
			genesisExported := erc20.ExportGenesis(nw.GetContext(), *erc20Keeper)
			params := nw.App.GetErc20Keeper().GetParams(nw.GetContext())
