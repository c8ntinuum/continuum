...
func (s *KeeperTestSuite) TestApplyTransaction() {
	...
	for _, tc := range testCases {
		s.Run(fmt.Sprintf("Case %s", tc.name), func() {
			...
			s.Require().NoError(s.Network.NextBlock())
			balanceAfterNextBlock := s.Network.App.GetBankKeeper().GetBalance(s.Network.GetContext(), s.Keyring.GetAccAddr(0), "aatom")
			s.Require().Equal(balanceAfterRefund.String(), balanceAfterNextBlock.String())
		})
	}
}
