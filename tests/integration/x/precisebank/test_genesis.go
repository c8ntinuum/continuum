...
func (s *GenesisTestSuite) setModuleBalance(amount sdkmath.Int) {
	ctx := s.network.GetContext()
	bankKeeper := s.network.App.GetBankKeeper()
	moduleAddr := s.network.App.GetAccountKeeper().GetModuleAddress(types.ModuleName)
	current := bankKeeper.GetBalance(ctx, moduleAddr, types.IntegerCoinDenom()).Amount

	switch {
	case current.LT(amount):
		err := bankKeeper.MintCoins(
			ctx,
			types.ModuleName,
			sdk.NewCoins(sdk.NewCoin(types.IntegerCoinDenom(), amount.Sub(current))),
		)
		s.Require().NoError(err)
	case current.GT(amount):
		err := bankKeeper.BurnCoins(
			ctx,
			types.ModuleName,
			sdk.NewCoins(sdk.NewCoin(types.IntegerCoinDenom(), current.Sub(amount))),
		)
		s.Require().NoError(err)
	}
}
...
func (s *GenesisTestSuite) TestInitGenesis() {
	duplicateAddr := sdk.AccAddress{1}.String()
	...
	tests := []struct {
		...
		},
		{
			...
			func() {
				s.setModuleBalance(sdkmath.NewInt(2))
			},
		...
		},
		{
			...
			"failed to validate precisebank genesis state: invalid balances: duplicate address " + duplicateAddr,
		},
		{
			"invalid - module balance insufficient",
			func() {
				s.setModuleBalance(sdkmath.ZeroInt())
			},
			types.NewGenesisState(
				types.FractionalBalances{
					types.NewFractionalBalance(sdk.AccAddress{1}.String(), types.ConversionFactor().SubRaw(1)),
		...
		},
		{
			...
			func() {
				s.setModuleBalance(sdkmath.NewInt(100))
			},
			types.NewGenesisState(
				types.FractionalBalances{
					types.NewFractionalBalance(sdk.AccAddress{1}.String(), types.ConversionFactor().SubRaw(1)),
	...
	tests := []struct {
		...
		},
		{
			...
			func() *types.GenesisState {
				s.setModuleBalance(sdkmath.OneInt())
