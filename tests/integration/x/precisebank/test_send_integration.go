...
func (s *KeeperIntegrationTestSuite) TestSendCoinsFromAccountToModuleMatchingErrors() {
	// x/bank panics on missing module accounts. x/precisebank should return an
	// error instead so operator misconfiguration does not halt the chain.
	...
	tests := []struct {
		...
		// SendCoinsFromAccountToModule specific missing-module behavior
		{
	...
	for _, tt := range tests {
		s.Run(tt.name, func() {
			...
			s.Require().PanicsWithError(tt.wantErr, func() {
				...
				s.Require().Error(err)
			}, "x/bank should still panic on missing module accounts")
...
func (s *KeeperIntegrationTestSuite) TestSendCoinsFromModuleToAccountMatchingErrors() {
	...
	tests := []struct {
		...
		// SendCoinsFromModuleToAccount specific missing-module behavior
		{
			"missing module account - passthrough",
			"cat",
			sdk.AccAddress([]byte{2}),
		...
		s.Run(tt.name, func() {
			...
			if tt.wantErr != "" {
				if tt.senderModule == "cat" {
					...
					s.Require().EqualError(pbankErr, tt.wantErr)
					return
				}
