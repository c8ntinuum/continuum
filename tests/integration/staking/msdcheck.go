...
func TestMSDCheckIntegration(t *testing.T, create network.CreateEvmApp, options ...network.ConfigOption) {
	...
	t.Run("MsgEditValidator still works when current stake falls below minimum", func(t *testing.T) {
		nw, tf, kr := setupNetwork(t, create, options...)
		...
		ctx := nw.GetContext()
		validator, err := nw.App.GetStakingKeeper().GetValidator(ctx, valAddr)
		require.NoError(t, err)

		validator.Tokens = minSelfDelegation.SubRaw(1)
		err = nw.App.GetStakingKeeper().SetValidator(ctx, validator)
		require.NoError(t, err)
		...
		res, err = commitCosmosTx(tf, kr.GetPrivKey(0), editMsg)
		require.NoError(t, err)
		require.Zerof(t, res.Code, "log: %s", res.Log)
	})

	t.Run("MsgEditValidator still rejects proposed min self delegation below minimum", func(t *testing.T) {
		nw, tf, kr := setupNetwork(t, create, options...)
		_ = nw
