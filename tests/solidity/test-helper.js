...
function extractChainIDFromGo(goFilePath) {
  try {
    if (!fs.existsSync(goFilePath)) {
      logger.warn(`Go config file not found at ${goFilePath}, using default chain ID: 2184`)
      return 2184
    }
    ...
    logger.warn('DefaultEVMChainID not found in Go file, using default: 2184')
    return 2184
  } catch (error) {
    logger.warn(`Error reading Go config file: ${error.message}, using default: 2184`)
    return 2184
  }
}
...
function setupNetwork({ runConfig, timeout }) {
  ...
  const spawnPromise = new Promise((resolve, reject) => {
    ...
    const serverStartedMsg = 'ctmd started'
    ...
    logger.info(`Starting ctmd process... timeout: ${timeout}ms`)
  ...
  const timeoutPromise = new Promise((resolve, reject) => {
    setTimeout(() => reject(new Error('Start ctmd timeout!')), timeout)
  })
