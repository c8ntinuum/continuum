...
func NewKeeper(cdc codec.BinaryCodec, storeKey storetypes.StoreKey, authority sdk.AccAddress) Keeper {
	if err := sdk.VerifyAddressFormat(authority); err != nil {
		panic(err) //nolint:halt // Constructor guard: circuit authority wiring must be valid at boot.
	}
	return Keeper{
		cdc:       cdc,
		storeKey:  storeKey,
...
func (k Keeper) GetSystemAvailable(ctx sdk.Context) bool {
	store := ctx.KVStore(k.storeKey)
...
func (k Keeper) SetSystemAvailable(ctx sdk.Context, available bool) {
	store := ctx.KVStore(k.storeKey)
	if available {
		store.Set(types.KeyState, []byte{1})
		return
	}
	store.Set(types.KeyState, []byte{0})
}
