...
Authority authorization is evaluated on the canonical decoded bech32 address,
not raw input string casing, so equivalent uppercase bech32 authority input is
...
2. `x/ibcratelimiterext/keeper/msg_server_test.go`

- validates authority authorization
- validates canonicalized authority input is accepted
- validates whitelisted operator authorization
- validates unauthorized signer rejection
