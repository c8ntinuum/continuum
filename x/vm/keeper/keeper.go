...
func NewKeeper(
	...
	if addr := ak.GetModuleAddress(types.ModuleName); addr == nil {
		panic("the EVM module account has not been set") //nolint:halt // Constructor guard: x/vm requires its module account to exist at boot.
	}

	// ensure the authority account is correct
	if err := sdk.VerifyAddressFormat(authority); err != nil {
		panic(err) //nolint:halt // Constructor guard: x/vm authority wiring must be valid at boot.
	}
	...
	if err := types.SetChainConfig(ethCfg); err != nil {
		panic(err) //nolint:halt // Boot invariant: chain config initialization must succeed exactly once.
	}
...
func (k *Keeper) SetHooks(eh types.EvmHooks) *Keeper {
	if k.hooks != nil {
		panic("cannot set evm hooks twice") //nolint:halt // One-time registration invariant: EVM hooks must only be installed once.
	}
