...
func setupEvmCoinInfo() {
	evmtypes.SetDefaultEvmCoinInfo(testconstants.ExampleChainCoinInfo[testconstants.SixDecimalsChainID])
}

func TestGenesisStateValidate_Basic(t *testing.T) {
	setupEvmCoinInfo()
	duplicateAddr := sdk.AccAddress{1}.String()
	negativeAddr := sdk.AccAddress{2}.String()
	...
	testCases := []struct {
		...
		},
		{
			...
			"invalid balances: duplicate address " + duplicateAddr,
		},
		{
			...
			"invalid balances: invalid fractional balance for " + negativeAddr + ": non-positive amount -1",
		},
		{
