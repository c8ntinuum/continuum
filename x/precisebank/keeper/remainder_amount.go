...
func (k *Keeper) GetRemainderAmount(
	...
	if err := bal.Unmarshal(bz); err != nil {
		panic(fmt.Errorf("failed to unmarshal remainder amount: %w", err)) //nolint:halt // Store invariant: precisebank remainder rows must remain decodable once committed.
	}
...
func (k *Keeper) SetRemainderAmount(
	...
	if err := types.ValidateFractionalAmount(amount); err != nil {
		panic(fmt.Errorf("remainder amount is invalid: %w", err)) //nolint:halt // Store invariant: remainder values must stay normalized to the fractional range.
	}
	...
	if err != nil {
		panic(fmt.Errorf("failed to marshal remainder amount: %w", err)) //nolint:halt // Store invariant: remainder values must remain serializable.
	}
