...
func (k *Keeper) GetFractionalBalance(ctx sdk.Context, address sdk.AccAddress) sdkmath.Int {
	...
	if err := bal.Unmarshal(bz); err != nil {
		panic(fmt.Errorf("failed to unmarshal fractional balance: %w", err)) //nolint:halt // Store invariant: fractional balance rows must remain decodable once committed.
	}
...
func (k *Keeper) SetFractionalBalance(
	...
	if address.Empty() {
		panic(errors.New("address cannot be empty")) //nolint:halt // Internal invariant: fractional balances are keyed by concrete account addresses.
	}
	...
	if err := types.ValidateFractionalAmount(amount); err != nil {
		panic(fmt.Errorf("amount is invalid: %w", err)) //nolint:halt // Store invariant: only normalized fractional amounts may be persisted.
	}
	...
	if err != nil {
		panic(fmt.Errorf("failed to marshal fractional balance: %w", err)) //nolint:halt // Store invariant: fractional balances must remain serializable.
	}
...
func (k *Keeper) IterateFractionalBalances(
	...
	for ; iterator.Valid(); iterator.Next() {
		...
		if err := amount.Unmarshal(iterator.Value()); err != nil {
			panic(fmt.Errorf("failed to unmarshal fractional balance: %w", err)) //nolint:halt // Store invariant: iterated fractional balance rows must remain decodable.
		}
