...
func (k Keeper) MintCoins(goCtx context.Context, moduleName string, amt sdk.Coins) error {
	...
	if moduleName == types.ModuleName {
		panic(errorsmod.Wrapf(sdkerrors.ErrUnauthorized, "module account %s cannot be minted to", moduleName)) //nolint:halt // Internal invariant: reserve module minting must never route through the public bank facade.
	}
	...
	if acc == nil {
		panic(errorsmod.Wrapf(sdkerrors.ErrUnknownAddress, "module account %s does not exist", moduleName)) //nolint:halt // Internal invariant: only registered module accounts may mint through precisebank.
	}

	if !acc.HasPermission(authtypes.Minter) {
		panic(errorsmod.Wrapf(sdkerrors.ErrUnauthorized, "module account %s does not have permissions to mint tokens", moduleName)) //nolint:halt // Internal invariant: precisebank mint callers must use a module with minter permission.
	}
