...
func (k Keeper) sendExtendedCoins(
	...
	if !senderNeedsBorrow && recipientNeedsCarry {
		...
		if err := k.bk.SendCoins(
			...
			panic(fmt.Errorf("failed to carry fractional coins to %s: %w", to, err)) //nolint:halt // Internal invariant: reserve carry must remain fully backed by precisebank accounting.
		}
	}
...
func subFromFractionalBalance(
	...
	if currentFractionalBalance.GTE(types.ConversionFactor()) {
		panic("currentFractionalBalance must be less than ConversionFactor") //nolint:halt // Arithmetic invariant: normalized fractional balances must stay below the conversion factor.
	}

	if amountToSub.GTE(types.ConversionFactor()) {
		panic("amountToSub must be less than ConversionFactor") //nolint:halt // Arithmetic invariant: fractional subtraction helper only accepts normalized inputs.
	}
...
func addToFractionalBalance(currentFractionalBalance sdkmath.Int, amountToAdd sdkmath.Int) (sdkmath.Int, bool) {
	...
	if amountToAdd.GTE(types.ConversionFactor()) {
		panic("amountToAdd must be less than ConversionFactor") //nolint:halt // Arithmetic invariant: fractional addition helper only accepts normalized inputs.
	}
...
// An error is returned if the recipient module account does not exist, if the
// recipient module is the x/precisebank module account, or if sending the tokens fails.
...
// An error is returned if the sender module account does not exist, if the
// recipient address is blocked, if the sender is the x/precisebank module
...
// An error is returned if either module account does not exist, if the
// recipient module is the x/precisebank module account, or if sending the
// tokens fails.
