...
func TestSendCoinsFromAccountToModule_MissingModuleReturnsError(t *testing.T) {
	td := newMockedTestData(t)
	td.ak.EXPECT().
		GetModuleAccount(td.ctx, "missing").
		Return(nil).
		Once()

	fromAddr := sdk.AccAddress([]byte{1})
	err := td.keeper.SendCoinsFromAccountToModule(td.ctx, fromAddr, "missing", cs(c("busd", 1000)))

	require.Error(t, err)
	require.EqualError(t, err, "module account missing does not exist: unknown address")
}

func TestSendCoinsFromModuleToAccount_MissingModuleReturnsError(t *testing.T) {
	td := newMockedTestData(t)
	td.ak.EXPECT().
		GetModuleAddress("missing").
		Return(nil).
		Once()

	toAddr := sdk.AccAddress([]byte{1})
	err := td.keeper.SendCoinsFromModuleToAccount(td.ctx, "missing", toAddr, cs(c("busd", 1000)))

	require.Error(t, err)
	require.EqualError(t, err, "module account missing does not exist: unknown address")
}

func TestSendCoinsFromModuleToModule_MissingModuleReturnsError(t *testing.T) {
	td := newMockedTestData(t)
	td.ak.EXPECT().
		GetModuleAddress("missing_sender").
		Return(nil).
		Once()

	err := td.keeper.SendCoinsFromModuleToModule(td.ctx, "missing_sender", "missing_recipient", cs(c("busd", 1000)))
	require.Error(t, err)
	require.EqualError(t, err, "module account missing_sender does not exist: unknown address")

	td = newMockedTestData(t)
	td.ak.EXPECT().
		GetModuleAddress("sender").
		Return(sdk.AccAddress{1}).
		Once()
	td.ak.EXPECT().
		GetModuleAccount(td.ctx, "missing_recipient").
		Return(nil).
		Once()

	err = td.keeper.SendCoinsFromModuleToModule(td.ctx, "sender", "missing_recipient", cs(c("busd", 1000)))
	require.Error(t, err)
	require.EqualError(t, err, "module account missing_recipient does not exist: unknown address")
}
