...
func (k Keeper) BurnCoins(goCtx context.Context, moduleName string, amt sdk.Coins) error {
	...
	if moduleName == types.ModuleName {
		panic(errorsmod.Wrapf(sdkerrors.ErrUnauthorized, "module account %s cannot be burned from", moduleName)) //nolint:halt // Internal invariant: reserve module burns must never route through the public bank facade.
	}

	// Panic errors are identical to x/bank for consistency.
	acc := k.ak.GetModuleAccount(ctx, moduleName)
	if acc == nil {
		panic(errorsmod.Wrapf(sdkerrors.ErrUnknownAddress, "module account %s does not exist", moduleName)) //nolint:halt // Internal invariant: only registered module accounts may burn through precisebank.
	}

	if !acc.HasPermission(authtypes.Burner) {
		panic(errorsmod.Wrapf(sdkerrors.ErrUnauthorized, "module account %s does not have permissions to burn tokens", moduleName)) //nolint:halt // Internal invariant: precisebank burn callers must use a module with burner permission.
	}
