...
var minSelfDelegation = func() math.Int {
	bi, _ := new(big.Int).SetString(minSelfDelegationStr, 10)
...
func (m *MsgServer) EditValidator(ctx context.Context, msg *stakingtypes.MsgEditValidator) (*stakingtypes.MsgEditValidatorResponse, error) {
	...
	validator, err := m.keeper.GetValidator(sdkCtx, valAddr)
	if err != nil {
		return nil, err
	}

	if validator.Tokens.LT(minSelfDelegation) {
		return nil, errorsmod.Wrapf(
			sdkerrors.ErrInvalidRequest,
			"validator stake %s < minimum %s",
