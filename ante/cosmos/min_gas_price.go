...
func (mpd MinGasPriceDecorator) AnteHandle(ctx sdk.Context, tx sdk.Tx, simulate bool, next sdk.AnteHandler) (newCtx sdk.Context, err error) {
	...
	if ctx.IsCheckTx() || ctx.IsReCheckTx() {
		minGasPriceAppToml := ctx.MinGasPrices()
		minGasPriceAppTomlVal := minGasPriceAppToml.AmountOf(evmDenom)
		if minGasPriceAppTomlVal.GT(minGasPrice) {
			minGasPrice = minGasPriceAppTomlVal
		}
	}
	// END c8ntinuum
