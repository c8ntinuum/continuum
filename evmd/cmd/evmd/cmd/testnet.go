...
func NewTestnetCmd(mbm module.BasicManager, genBalIterator banktypes.GenesisBalancesIterator, appCreator appCreator) *cobra.Command {
	...
	// If the binary is built with the local-validator start tag, unsafeStartValidatorFn will be set
...
Example:
	ctmd testnet init-files --v 4 --output-dir ./.testnets --starting-ip-address 192.168.10.2
	`,
	...
	cmd.Flags().String(flagNodeDaemonHome, "ctmd", "Home directory of the node's daemon configuration")
...
Example:
	ctmd testnet --v 4 --output-dir ./.testnets
	`,
		RunE: func(cmd *cobra.Command, _ []string) error {
			args := startArgs{}
			args.outputDir, _ = cmd.Flags().GetString(flagOutputDir)
...
// NewTestNetworkFixture returns a new ctmd AppConstructor for network simulation tests
