...
const flagUnsafeAddedStoreKeys = "unsafe-added-store-keys"

func normalizeStoreKeys(in []string) []string {
...
func loadAppForExport(app *evmd.EVMD, height int64, addedStoreKeys []string) error {
	addedStoreKeys = normalizeStoreKeys(addedStoreKeys)

	if len(addedStoreKeys) == 0 {
		if height != -1 {
	...
	if height != -1 {
		if err := app.CommitMultiStore().LoadVersionAndUpgrade(height, upgrades); err != nil {
			return fmt.Errorf("failed to load version %d with store upgrades: %w", height, err)
		}
	} else {
		if err := app.CommitMultiStore().LoadLatestVersionAndUpgrade(upgrades); err != nil {
			return fmt.Errorf("failed to load latest version with store upgrades: %w", err)
		}
	}
...
// NewRootCmd creates a new root command for ctmd. It is called once in the
// main function.
...
func initRootCmd(rootCmd *cobra.Command, evmApp *evmd.EVMD) {
	...
	rootCmd.AddCommand(
		...
		genesisCommands(evmApp.TxConfig(), evmApp.BasicModuleManager, evmApp.AppCodec(), defaultNodeHome),
	...
	rootCmd.PersistentFlags().StringSlice(
		flagUnsafeAddedStoreKeys,
		nil,
		"Comma-separated store keys to add only while loading state for export (example: breaker). Export-only escape hatch; ignored by normal start.",
	)
}
...
func appExport(
	...
	addedStoreKeys := normalizeStoreKeys(cast.ToStringSlice(appOpts.Get(flagUnsafeAddedStoreKeys)))

	exampleApp = evmd.NewExampleApp(logger, db, traceStore, false, appOpts, baseapp.SetChainID(chainID))

	if err := loadAppForExport(exampleApp, height, addedStoreKeys); err != nil {
		return servertypes.ExportedApp{}, err
	}
