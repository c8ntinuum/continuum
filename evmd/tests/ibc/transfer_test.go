...
func (suite *TransferTestSuite) TestHandleMsgTransferBlockedWhenIbcUnavailable() {
	pathAToB := evmibctesting.NewTransferPath(suite.evmChainA, suite.chainB)
	pathAToB.Setup()

	evmApp := suite.evmChainA.App.(*evmd.EVMD)
	evmApp.IbcBreakerKeeper.SetIbcAvailable(suite.evmChainA.GetContext(), false)

	sourceDenom, err := evmApp.StakingKeeper.BondDenom(suite.evmChainA.GetContext())
	suite.Require().NoError(err)

	senderIdx := 1
	sender := suite.evmChainA.SenderAccounts[senderIdx]
	timeoutHeight := clienttypes.NewHeight(1, 110)

	msg := types.NewMsgTransfer(
		pathAToB.EndpointA.ChannelConfig.PortID,
		pathAToB.EndpointA.ChannelID,
		sdk.NewCoin(sourceDenom, sdkmath.NewInt(5)),
		sender.SenderAccount.GetAddress().String(),
		suite.chainB.SenderAccount.GetAddress().String(),
		timeoutHeight,
		0,
		"",
	...
	_, err = suite.evmChainA.SendMsgsWithSender(sender, msg)
