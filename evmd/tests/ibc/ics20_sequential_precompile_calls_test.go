...
func (suite *ICS20SequentialPrecompileCallsTestSuite) TestReceiveAndSendTwice() {
	...
	//    - Attempt two ICS20 sends
	//    - Complete without reverting in the current ICS20 flow
	...
	// Current behavior: tx succeeds and tokens remain on the sender contract.
	suite.Require().NoError(err)
	suite.Require().False(hasRevertEvent, "did not expect transaction revert")
	suite.Require().Equal("0", senderBalAfter.String(), "sender balance should be consumed by transferFrom")
	suite.Require().Equal(big.NewInt(SeqICS20InitialTokenAmount).String(), contractBalAfter.String(), "contract should retain transferred tokens")
}
