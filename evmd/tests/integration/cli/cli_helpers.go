...
func SubmitWhitelistProposal(
	...
	_ = ExecCLICmd(t, clientCtx, rootCmd,
	...
	WaitForTx(t, netw, clientCtx, ExtractTxHash(t, depositOut))
	require.NoError(t, netw.WaitForNextBlock())

	voteOut := ExecCLICmd(t, clientCtx, rootCmd,
		"tx", "gov", "vote", strconv.FormatUint(proposalID, 10), "yes",
		"--from", clientCtx.FromName,
		"--chain-id", chainID,
		"--fees", feeAmount,
		"--broadcast-mode", "sync",
	...
	WaitForTx(t, netw, clientCtx, ExtractTxHash(t, voteOut))
	require.NoError(t, netw.WaitForNextBlock())
	...
	for i := 0; i < 50; i++ {
		...
		if resp.Proposal.Status == govv1.StatusPassed {
			passed = true
			break
		}
	}
	require.Truef(t, passed, "proposal did not pass, last status: %s, reason: %s, tally: %+v", lastStatus.String(), lastReason, lastTally)
