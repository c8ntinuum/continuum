...
func initGenFiles(cfg Config, genAccounts []authtypes.GenesisAccount, genBalances []banktypes.Balance, genFiles []string) error {
	...
	for _, bal := range genBalances {
		totalSupply = totalSupply.Add(bal.Coins...)
	}
	bankGenState.Supply = totalSupply
	if cfg.InjectDenomMetadata && !hasDenomMetadata(bankGenState.DenomMetadata, cfg.BondDenom) {
		bankGenState.DenomMetadata = append(bankGenState.DenomMetadata, defaultDenomMetadata(cfg.BondDenom))
	}
	...
	evmGenState.Params.ExtendedDenomOptions = &evmtypes.ExtendedDenomOptions{ExtendedDenom: cfg.BondDenom}
...
func hasDenomMetadata(metas []banktypes.Metadata, denom string) bool {
...
func defaultDenomMetadata(denom string) banktypes.Metadata {
	return banktypes.Metadata{
		Description: "Native 18-decimal denom metadata for c8ntinuum chain",
		Base:        denom,
