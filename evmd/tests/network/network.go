...
import (
	...
	appconfig "github.com/cosmos/evm/config"
...
type Config struct {
	...
	GentxSelfDelegation      *math.Int                // optional min self-delegation for gentx (defaults to bonded tokens)
	...
	InjectDenomMetadata      bool   // inject bank denom metadata for BondDenom into genesis
}
...
type (
	...
	// or handler.
	Validator struct {
		AppConfig     *evmserverconfig.Config
...
func New(l Logger, baseDir string, cfg Config) (*Network, error) {
	...
	for i := 0; i < cfg.NumValidators; i++ {
		appCfg := evmserverconfig.DefaultConfig()
		...
		if i == 0 {
			...
			appCfg.JSONRPC.API = evmserverconfig.GetAPINamespaces()
		}
		...
		minSelfDelegation := bondedTokens
		if cfg.GentxSelfDelegation != nil {
			minSelfDelegation = *cfg.GentxSelfDelegation
		}
		...
		customAppTemplate, _ := appconfig.InitAppConfig(testconstants.ExampleAttoDenom, testconstants.ExampleEIP155ChainID)
