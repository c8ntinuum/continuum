...
import (
	...
	valrewardskeeper "github.com/cosmos/evm/x/valrewards/keeper"
...
var ctmdMsdTxPrefix = []byte{0x08, 0x08, 0x08, 0x08}
...
type EVMD struct {
	*baseapp.BaseApp
	...
	// c8ntinuum keepers
	ValRewardsKeeper valrewardskeeper.Keeper
	...
	// c8ntinuum
	msdViolationBuffer *msd.MsdViolationBuffer
	// END c8ntinuum
}
...
func NewExampleApp(
	...
	keys := storetypes.NewKVStoreKeys(
		...
		// c8ntinuum store keys
		valrewardstypes.StoreKey,
	)
	...
	// c8ntinuum
	app.msdViolationBuffer = msd.NewMsdViolationBuffer()
	// END c8ntinuum
	...
	*/
	...
	// Set up c8ntinuum validators rewards keeper
	app.ValRewardsKeeper = valrewardskeeper.NewKeeper(
		keys[valrewardstypes.StoreKey],
		app.StakingKeeper,
	)
	...
	*/
	...
	app.ModuleManager = module.NewManager(
		...
		// c8ntinuum modules
		valrewards.NewAppModule(app.ValRewardsKeeper, app.AccountKeeper),
	)
	...
	app.BasicModuleManager = module.NewBasicManagerFromManager(
		...
		map[string]module.AppModuleBasic{
			...
			// c8ntinuum validator rewards add module basic
			valrewardstypes.ModuleName: valrewards.AppModuleBasic{},
		},
	)
	...
	app.SetVerifyVoteExtensionHandler(app.verifyVoteExtHandler())

	msd.Init(app.StakingKeeper, app.SlashingKeeper, evmtypes.DefaultEVMDenom, app.appCodec)
	// END c8ntinuum
...
func (app *EVMD) getVoteExtensionAndQuorum(req *abci.RequestPrepareProposal) ([]byte, bool) {
	var myExt []byte
	var total, signed, ext int64
	for _, v := range req.LocalLastCommit.Votes {
		total += v.Validator.Power
		if bytes.Equal(v.Validator.Address, req.ProposerAddress) {
			myExt = v.GetVoteExtension()
		}
		if v.BlockIdFlag == cmtproto.BlockIDFlagCommit {
			signed += v.Validator.Power
			if len(v.GetVoteExtension()) > 0 {
				ext += v.Validator.Power
			}
		}
	}
	hasTwoThirdsCommit := signed > (total*2)/3
	hasTwoThirdsExt := ext > (total*2)/3
	return myExt, hasTwoThirdsCommit && hasTwoThirdsExt
}

func (app *EVMD) prepareProposalHandler() sdk.PrepareProposalHandler {

	return func(ctx sdk.Context, req *abci.RequestPrepareProposal) (*abci.ResponsePrepareProposal, error) {

		// Init final prepare txs list
		var txList [][]byte = [][]byte{}

		// Check MSD vote extension and if powered enough create kick tx
		ve, isVoted := app.getVoteExtensionAndQuorum(req)
		if len(ve) > 0 && isVoted {
			var msdViolation msd.MSDViolation
			if err := json.Unmarshal(ve, &msdViolation); err == nil {
				voteOutMSDViolatorTx := append(ctmdMsdTxPrefix, []byte(msdViolation.ProposerHex)...)
				txList = append(txList, voteOutMSDViolatorTx)
			}
		}

		// Iterate transactions
		for i, txBz := range req.Txs {
			// Ensure kick tx is skipped for verifications
			if bytes.HasPrefix(txBz, ctmdMsdTxPrefix) {
				continue
			}
			// Decode tx
			tx, err := app.TxDecode(txBz)
			if err != nil {
				continue
			}
			// Check MSD violations
			if err := msd.CheckMSDInTx(ctx, tx); err != nil {
				app.Logger().Error("prepare proposal is dropping tx due to MSD verification", "index", i, "reason", err.Error())
				continue
			}
			// If all ok, add tx to block list
			txList = append(txList, txBz)
		}
		return &abci.ResponsePrepareProposal{Txs: txList}, nil
	}
}

func (app *EVMD) processProposalHandler() sdk.ProcessProposalHandler {
	...
	return func(ctx sdk.Context, req *abci.RequestProcessProposal) (*abci.ResponseProcessProposal, error) {
		...
		for i, txBz := range req.Txs {
			...
			tx, err := app.TxDecode(txBz)
			if err != nil {
				app.Logger().Error("process proposal tx decode failed; rejecting block", "index", i, "err", err)
				return &abci.ResponseProcessProposal{Status: abci.ResponseProcessProposal_REJECT}, nil
			}

			// Check MSD violations
			if err := msd.CheckMSDInTx(ctx, tx); err != nil {
				h := fmt.Sprintf("%X", req.Hash)
				propHex := fmt.Sprintf("%X", req.ProposerAddress)
				app.msdViolationBuffer.Record(req.Height, req.ProposedLastCommit.Round, propHex, h, "MSD")
				ctx.Logger().Error("process proposal MSD violation → RECORDED (will reject block)", "height", req.Height, "round", req.ProposedLastCommit.Round, "proposer", propHex, "hash", h)
				return &abci.ResponseProcessProposal{Status: abci.ResponseProcessProposal_REJECT}, nil
			}
		}
...
func (app *EVMD) extendVoteHandler() sdk.ExtendVoteHandler {
	return func(ctx sdk.Context, req *abci.RequestExtendVote) (*abci.ResponseExtendVote, error) {
		msdViolations, bz := app.msdViolationBuffer.ForHeight(req.Height)
		if msdViolations != nil {
...
func (app *EVMD) verifyVoteExtHandler() sdk.VerifyVoteExtensionHandler {
	const maxVEBytes = 256

	return func(ctx sdk.Context, req *abci.RequestVerifyVoteExtension) (*abci.ResponseVerifyVoteExtension, error) {

		if len(req.VoteExtension) > maxVEBytes {
			return &abci.ResponseVerifyVoteExtension{Status: abci.ResponseVerifyVoteExtension_REJECT}, fmt.Errorf("vote extension too large: %d", len(req.VoteExtension))
		}
		...
		var msdViolation msd.MSDViolation
		if err := json.Unmarshal(req.VoteExtension, &msdViolation); err != nil {
			return &abci.ResponseVerifyVoteExtension{Status: abci.ResponseVerifyVoteExtension_REJECT}, fmt.Errorf("invalid VE json: %w", err)
		}

		if msdViolation.Height != req.Height {
			return &abci.ResponseVerifyVoteExtension{Status: abci.ResponseVerifyVoteExtension_REJECT},
				fmt.Errorf("verify vote handler height mismatch: got %d, want %d", msdViolation.Height, req.Height)
		}
		return &abci.ResponseVerifyVoteExtension{Status: abci.ResponseVerifyVoteExtension_ACCEPT}, nil
	}
}
...
func (app *EVMD) PreBlocker(ctx sdk.Context, req *abci.RequestFinalizeBlock) (*sdk.ResponsePreBlock, error) {
	if ctx.ConsensusParams().Abci.VoteExtensionsEnableHeight > 0 && len(req.Txs) > 0 {
		for _, txBz := range req.Txs {
			if bytes.HasPrefix(txBz, ctmdMsdTxPrefix) {
				if err := msd.Jail(ctx, string(txBz[4:])); err != nil {
					ctx.Logger().Error("pre blocker jail failed for malicious proposer", "err", err, "proposer", string(txBz[4:]))
					continue
				}
				if err := msd.Slash(ctx, string(txBz[4:]), req.Height); err != nil {
					ctx.Logger().Error("pre blocker slash failed for malicious proposer", "err", err, "proposer", string(txBz[4:]))
					continue
				}
			}
		}
