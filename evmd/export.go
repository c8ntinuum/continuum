...
// synthesizes a best-effort row so that zero-height export can continue. This is
// unsafe in the sense that exact old reward accounting cannot be recovered once
// the referenced historical row has been lost.
func (app *EVMD) repairDistrReferenceCounts(ctx sdk.Context) error {
	...
	bump := func(val sdk.ValAddress, period uint64) error {
		...
		// SDK invariant: refcount should never exceed 2.
