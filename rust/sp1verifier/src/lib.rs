...
fn guard_ffi_boundary<F>(f: F) -> c_int
where
    F: FnOnce() -> c_int,
{
    panic::catch_unwind(AssertUnwindSafe(f)).unwrap_or(5)
}
...
#[cfg(test)]
mod tests {
    use super::guard_ffi_boundary;

    #[test]
    fn guard_ffi_boundary_maps_panics_to_internal_error() {
        assert_eq!(guard_ffi_boundary(|| panic!("boom")), 5);
    }

    #[test]
    fn guard_ffi_boundary_preserves_success_codes() {
        assert_eq!(guard_ffi_boundary(|| 3), 3);
    }
}
