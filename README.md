...
- **Heterogeneous interop**: Native support for IBC-style flows and rich cryptographic support.
...
Variable-length precompiles meter gas by input size and may reject oversized payloads to preserve block-production safety on live networks.
...
- **Advanced and post-quantum cryptography**: Precompiles for PQ signatures (`ML-DSA`, `SLH-DSA`), `Ed25519`, caller-supplied `FROST` signature verification, and multiple signature schemes/curves (`Schnorr`, `Schnorrkel`, `P-256`) for compatibility with existing systems.
...
- **Data & UX precompiles**: `json` and `bech32` for structured JSON parsing and bech32/EVM address bridging improve both developer ergonomics and end-user UX in multi-domain applications, with bounded parser inputs for validator safety.
...
| `WETH9`                     | Canonical wrapped-native token                      | `0xc8Fb80fCc03f699C70ff0CC08C09106288888888` | ERC-20 wrapper for the native token, making it usable in existing DeFi tools.  |
...
| --------------------------- | ------------------------------------------------ |
| **EVM HTTP RPC URL**        | `https://public-evm-rpc.c8ntinuum.com`           |
| **EVM WebSocket RPC URL**   | `wss://public-evm-rpc.c8ntinuum.com/websocket`   |
| **Comet HTTP RPC URL**      | `https://public-comet-rpc.c8ntinuum.com`         |
| **Coemt WebSocket RPC URL** | `wss://public-comet-rpc.c8ntinuum.com/websocket` |
| **Chain ID**                | `2184`                                           |
| **Currency Symbol**         | `CTM`                                            |
